...
#define GET_PLAYER_NAME(is_p1_turn)			((is_p1_turn) ? PLAYER1_NAME : PLAYER2_NAME)
...
#define MSG_TURN(is_p1_turn)				std::wstring(GET_PLAYER_NAME(is_p1_turn)) + L" 차례입니다!"
#define MSG_CANNOT_PLACE_THERE				L"그 위치에 배치할 수 없습니다."
#define MSG_WIN(is_p1_turn)					std::wstring(GET_PLAYER_NAME(is_p1_turn)) + L"이(가) 이겼습니다."
#define MSG_DECIDED_BALL(is_p1_turn, type)	std::wstring(GET_PLAYER_NAME(is_p1_turn)) + L"은(는) 앞으로 " + std::wstring(GET_BALL_TYPE(type)) + L"을 넣어야합니다."
